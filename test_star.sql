...
-- ponytail: plain SQL, no test framework. on every ReplacingMergeTree.

-- 1. Row counts.
SELECT t, n FROM (
  SELECT 'dim_customer' t, count() n FROM ax.dim_customer
...
LEFT JOIN ax.dim_customer AS c  ON c.data_area = f.data_area AND c.customer_id = f.cust_account
LEFT JOIN ax.dim_item     AS i  ON i.data_area = f.data_area AND i.item_id = f.item_id
LEFT JOIN ax.dim_date     AS d        ON d.date_key = f.order_date
LEFT JOIN ax.dim_currency AS cu ON cu.currency_code = f.currency;
...
JOIN ax.dim_status   AS s ON s.status_type = 'trans_type' AND s.status_code = f.trans_type
