...
FROM ax.dim_customer AS c
JOIN (
  SELECT data_area, cust_account, sum(line_amount) AS revenue
  FROM ax.fact_sales WHERE data_area = 'usrt' GROUP BY 1, 2
...
JOIN (
  ...
  FROM ax.fact_cust_trans WHERE data_area = 'usrt' AND trans_type = 2 GROUP BY 1, 2
